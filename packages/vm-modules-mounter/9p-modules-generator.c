				 ...
				 const char *_9p_mountpoint)
{
	...
	char *_9p_unit_name = NULL;
	int ret;

	ret = mountpoint_to_unit_name(_9p_mountpoint, &_9p_unit_name);
	if (ret == -1)
		goto out;
	...
	ret = fprintf_new_file(unit_name,
			       ...
			       "Type=oneshot\n"
			       "RemainAfterExit=yes\n"
			       "ExecStart=/bin/ln -s build/modules.order /lib/modules/%2$s/modules.order\n"
			       "ExecStart=/bin/ln -s build/modules.builtin /lib/modules/%2$s/modules.builtin\n"
			       "ExecStart=/bin/ln -s build /lib/modules/%2$s/kernel\n"
			       "ExecStart=/bin/depmod %2$s\n",
			       _9p_unit_name, kernelrelease);
	if (ret == -1)
		goto out;
...
static int generate_units(void)
{
	...
	ret = generate_modules_unit(name.release, _9p_mountpoint);
	if (ret == -1)
		goto out;
